...
// $pageTitle = $weekDays[$selectedDay] . ' ' . $titleDate;
$pageTitle = $titleDate;
// ====== 時間帯グルーピング ======
$groupedPrograms = [
    'Morning'   => [],
...
echo "<!-- items=" . count($items) .
     ...
     " -->";
// 旧: ここで isNowOnAir を呼ぶ debug echo があったが、$it が未定義のため削除。
?>
<!DOCTYPE html>
<html lang="ja">
<head>
    <meta charset="UTF-8">
    <title><?php echo h($pageTitle); ?>｜番組表</title>
  <!-- レスポンシブ -->
...
<link rel="stylesheet" href="css/styles.css<?php echo $themeParam ?? ''; ?>">
...
(function () {
  ...
  window.addEventListener('load', function () {
    ...
    // デフォルトの自動スクロール（group-Daytime）は無効化
    // const groupDaytime = document.getElementById('group-Daytime');
    // if (groupDaytime) {
    //   groupDaytime.scrollIntoView({ behavior: 'auto', block: 'start' });
    // }
  });
})();
</script>
...
<div class="v0-card">
        ...
        <div class="v0-h1-wrap v0-h1-bracket">
          ...
          <div class="v0-h1-title-row">
            <div class="v0-h1-textgroup">
              <span class="v0-h1-subtitle">TIMETABLES</span>
              <h1 class="v0-h1-title">タイムテーブル</h1>
            </div>
    ...
    <!-- <div class="day-title-left"> -->
      <!-- <h1 class="day-title"><i class="fa-solid fa-radio"></i><?php //echo h($pageTitle); ?> 週間タイムテーブル</h1> -->
    <!-- </div> -->
  <!-- </div> -->
...
<div class="card nav-panel">
  ...
  <nav class="week-nav" aria-label="曜日ナビゲーション">
    ...
    <?php endforeach; ?>
    <?php if ($nowOnAirAnchor): ?>
      <!-- <a class="week-nav-link nowonair-link-button" href="<?php echo h($nowOnAirAnchor); ?>">
        放送中へ
      </a> -->
    <?php endif; ?>
  </nav>
  ...
  <nav class="time-nav" aria-label="時間帯ナビゲーション">
...
<!-- <h1 class="day-title"><?php echo h($pageTitle); ?> の番組表</h1> -->
...
<?php foreach ($groupedPrograms as $groupName => $programs): ?>
  ...
  <?php if (!empty($availableGroups)): ?>
  <nav class="time-nav" aria-label="時間帯ナビゲーション">
    <?php foreach ($availableGroups as $gName => $label): ?>
      <a href="#group-<?php echo h($gName); ?>" class="time-nav-link <?php echo ($groupName === $gName) ? 'active' : ''; ?>">
        <?php echo h($label); ?>
      </a>
    <?php endforeach; ?>
  </nav>
  ...
  <div id="group-<?php echo h($groupName); ?>" class="time-group"><?php //echo h($labels[$groupName] ?? $groupName); ?></div>
  ...
  ?>
    <?php
      ...
      // デバッグ用：元データを出力
      echo "<!-- DURATION_DEBUG: title=" . h($title) . 
           " onair_time=" . h($onairTimeStr) . 
           " onair_start=" . h($startStr) . 
           " onair_end=" . h($endStr) . 
           " onair_starttime=" . h($startTimeNum) . 
           " onair_endtime=" . h($endTimeNum) . " -->";
      ...
      if ($startStr !== '' && $endStr !== '') {
        ...
        echo "<!-- DURATION_CALC: startStr={$startStr} endStr={$endStr} startMin={$startMin} endMin={$endMin} durationMinutes={$durationMinutes} -->";
      } else {
        // onair_start/endが無い場合、onair_timeから計算を試みる
        if ($onairTimeStr !== '') {
          // "HH:MM - HH:MM" 形式をパース
          if (preg_match('/(\d{1,2}):(\d{2})\s*[-～〜]\s*(\d{1,2}):(\d{2})/u', $onairTimeStr, $m)) {
            ...
            $durationMinutes = $endMin - $startMin;
            echo "<!-- DURATION_CALC_FROM_TIME: onair_time={$onairTimeStr} startMin={$startMin} endMin={$endMin} durationMinutes={$durationMinutes} -->";
          }
        }
        
        // それでも無い場合、onair_starttime/endtimeから計算
        if ($durationMinutes === null && $startTimeNum !== '' && $endTimeNum !== '') {
          $st = (int)preg_replace('/\D/', '', $startTimeNum);
          ...
          $durationMinutes = $endMin - $startMin;
          echo "<!-- DURATION_CALC_FROM_NUM: starttime={$startTimeNum} endtime={$endTimeNum} startMin={$startMin} endMin={$endMin} durationMinutes={$durationMinutes} -->";
        }
        
        if ($durationMinutes === null) {
          echo "<!-- DURATION_CALC: FAILED - 時間データが見つかりません -->";
        }
      }
      
      // 9分以下かどうか（明示的にbooleanにキャスト）
      $isMini = ($durationMinutes !== null && $durationMinutes <= 9) ? true : false;
      echo "<!-- DURATION_RESULT: durationMinutes=" . ($durationMinutes !== null ? $durationMinutes : 'null') . 
           " isMini=" . ($isMini ? 'true' : 'false') . 
           " isMini_type=" . gettype($isMini) . 
           " isMini_value=" . var_export($isMini, true) . " -->";
      
      // サムネ（timetables/index.php用：通常はlogo_radiko優先、9分以下はlogo優先）
    ...
    <?php echo $isNow ? 'id="now-on-air"' : ''; ?>>
      ...
      <div class="program-row">
        ...
        <div class="program-info">
          <div class="program-time"><i class="fa-solid fa-clock"></i> 
            <?php echo h($time); ?>
            <?php if ($isNow): ?>
